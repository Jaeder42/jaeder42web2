<script>
  ...
  const dataPointsComp = points.map(point => ({
    x: xScale(parseTime(dataComp[point].date)),
    y: yScale(dataComp[point].value),
    value: dataComp[point].value
  }));
    ...
    <g transform="translate({margin.top} {margin.left})">
      ...
      <g
        ...
        <path
          ...
          stroke-width="1"
          d={line(data)} />
        <path fill="none" stroke="red" stroke-width="1" d={line(dataComp)} />
        <!-- area chart -->
        <path opacity="0.15" fill="currentColor" stroke="none" d={area(data)} />
        <path opacity="0.15" fill="red" stroke="none" d={area(dataComp)} />
      </g>
      ...
      {#each dataPointsComp as dataPoint}
        <circle
          fill="none"
          stroke="red"
          stroke-width="1"
      ...
      {#if tooltip}
        <g
          fill="currentColor"
          transform="translate({xScale(parseTime(tooltip.date))}
          {yScale(tooltip.value)})">
          <text
            text-anchor="middle"
            font-size="5"
            font-weight="bold"
      ...
      {#if tooltipComp}
        <g
          fill="currentColor"
          transform="translate({xScale(parseTime(tooltipComp.date))}
          {yScale(tooltipComp.value)})">
          <text
            text-anchor="middle"
            font-size="5"
            font-weight="bold"
          ...
          <path
            ...
            stroke-width="0.5"
            stroke-dasharray="1"
            d="M 0 0 v {height - yScale(tooltipComp.value)}" />
          <circle r="2" fill="hsl(0, 0%, 10%)" />
        </g>
      {/if}
      ...
      {#each data as dataPoint, index}
        <g transform="translate({xScale(parseTime(dataPoint.date))} 0)">
          ...
          <rect
            on:mouseenter={() => {
              tooltip = data[index];
              tooltipComp = dataComp[index];
            }}
            opacity="0"
            x="-{xScale(parseTime(data[1].date)) / 2}"
            width={xScale(parseTime(data[1].date)) - xScale(parseTime(data[0].date))}
            {height} />
        </g>
      {/each}
    </g>
